...
//   ② 没有可用空格时，**按 `、-)_>」』;~】:：` 的先后顺序**逐个符号匹配，命中即切
//      （同一符号取最靠后那一个）。顺序即优先级 —— `、` 永远优先于 `:`。
...
const CUT_SYMBOLS = '、-)_>」』;~】:：';  // : 与 ： 是 2026-09-25 按用户要求补上的切点
...
function stripOldNumber(text) {
    ...
    if (cut >= 0 && cut < end) {
        ...
        // 2026-09-25（用户给定）：**按符号表顺序逐个匹配** —— 先试表里第一个符号，命中就用它；
        // 没命中再试第二个，依此类推。**不是**「所有符号混在一起从后往前扫」：
        // 那种扫法加一个符号会抢走已有切点（实测：加 `：` 后，正文以 `：` 收尾的标题整条不再清理）。
        // 同一个符号内部仍取**最靠后**的那一个（保持原有行为）。
        cut = -1;
        for (const symbol of CUT_SYMBOLS) {
            const at = text.lastIndexOf(symbol, end - 1);
            if (at >= 0) { cut = at; break; }
        }
    }
